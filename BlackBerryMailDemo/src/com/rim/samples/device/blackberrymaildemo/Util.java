 ...
 * provides functionality to retrieve an icon associated with a message based
 * on the message status, format a Date in String format, and sort messages in 
...
public final class Util
{
     ...
     * 
     * @param message The message for which to retrieve an icon respresenting its status
     * @return The character icon associated with the mesage status, ' ' if none found
     *          
     */
    public static char getStatusIcon(Message message)
    {
        char icon = ' ';
     ...
     * @return The String representation of the provided date, in the specified style
     */
     ...
     * 
     * @param date A date instance
     * @param style One of the DateFormat styles (@see net.rim.device.api.i18n.DateFormat)
     * @return The String representation of the provided date, in the specified style
     */
    public static String getDateAsString(Date date, int style)
    {
        ...
        if (date != null)
        {
            DateFormat dateFormat = DateFormat.getInstance(style);        
    ...
    public final static Comparator SORT_BY_MOST_RECENT_DATE = new Comparator()
    {
        ...
        {            
            if( !(first instanceof Message) || !(second instanceof Message) )
                throw new IllegalArgumentException("Arguments to compare must be a Message");

            Date firstDate = ((Message) first).getSentDate();
            Date secondDate = ((Message) second).getSentDate();
